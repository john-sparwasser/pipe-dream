...
public class RomPrepTests
{
    ...
    private const string GoldenPrepV16Sha256 = "165b09f51a1b97bced2ee09927b96ec00188b8114dfb845ef6d1da197c22f98c";
    ...
    public void prepped_vanilla_matches_the_golden_hashes_for_both_versions()
    {
        ...
        try
        {
            ...
            File.Copy(TestRom.RealRomPath, tmp, overwrite: true);
            Assert.Null(RomPrep.PrepInPlace(tmp, version: 15));     // frozen V15 stamp list
            Assert.Equal(GoldenPrepV15Sha256, RomHash.HeaderlessSha256File(tmp));

            File.Copy(TestRom.RealRomPath, tmp, overwrite: true);
            Assert.Null(RomPrep.PrepInPlace(tmp, version: 16));     // frozen V16 stamp list
            Assert.Equal(GoldenPrepV16Sha256, RomHash.HeaderlessSha256File(tmp));

            File.Copy(TestRom.RealRomPath, tmp, overwrite: true);
            Assert.Null(RomPrep.PrepInPlace(tmp, version: 17));     // frozen V17 stamp list
            Assert.Equal(GoldenPrepV17Sha256, RomHash.HeaderlessSha256File(tmp));

            File.Copy(TestRom.RealRomPath, tmp, overwrite: true);
            Assert.Null(RomPrep.PrepInPlace(tmp, version: 18));     // frozen V18 stamp list
            ...
            File.Copy(TestRom.RealRomPath, tmp, overwrite: true);
            Assert.Null(RomPrep.PrepInPlace(tmp, version: 20));     // frozen V20 stamp list
            Assert.Equal(GoldenPrepV20Sha256, RomHash.HeaderlessSha256File(tmp));

            File.Copy(TestRom.RealRomPath, tmp, overwrite: true);
            Assert.Null(RomPrep.PrepInPlace(tmp));                  // current (V21)
            ...
            Assert.True(GoldenPrepV21Sha256 == cur, $"V21 golden hash is now {cur}");
        }
        finally { File.Delete(tmp); }
    }
